...
public class AlgorithmFrame extends JFrame {
    ...
    private class AlgoCanvas extends JPanel{
        ...
        public void paintComponent(Graphics g) {
            ...
            AlgorithmVisualHelper.setStrokeWidth(g2d, 3);
            AlgorithmVisualHelper.setColor(g2d, AlgorithmVisualHelper.Blue);
            AlgorithmVisualHelper.strokeCircle(g2d, circle.getX(), circle.getY(), circle.getR());
            for (int i=0; i<data.getPointNumber(); i++) {
                Point p = data.getPoint(i);
                if (circle.contain(p)) {
                    AlgorithmVisualHelper.setColor(g2d, AlgorithmVisualHelper.Red);
                } else {
                    AlgorithmVisualHelper.setColor(g2d, AlgorithmVisualHelper.Green);
                }
                AlgorithmVisualHelper.fillCircle(g2d, p.x, p.y, 1);
            }
        }
